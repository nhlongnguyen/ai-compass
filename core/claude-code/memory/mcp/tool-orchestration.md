...
Auto_MCP_Integration:
  analyze_requests → Sequential for complex analysis + Context7 for research
  build_requests → Magic(UI generation) if frontend components needed
  design_requests → Sequential(system design) + Context7(architecture patterns)
  explain_requests → Context7 if external libraries mentioned, otherwise native
  improve_requests → Sequential(analysis) → Context7(best practices)
  troubleshoot_requests → Sequential(root cause) → Context7(solutions)

Flag_Override_Examples:
  "build react component --magic --c7" → UI generation + documentation lookup
  "analyze performance --seq --pup" → Performance analysis + browser testing
  "design API --seq --no-mcp" → System design with native tools only
```
...
Priority_Order:
  ...
  2. Auto-activation based on request + context
