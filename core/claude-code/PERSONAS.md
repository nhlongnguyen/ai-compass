# PERSONAS.md - Cognitive Specialization Modes
...
> `/persona:<name>` → Cognitive Mode Activation
...
### Persona-Optimized Commands

```yaml
architect:
  /user:design --api --ddd --think-hard
  /user:estimate --complexity --architecture
  /user:analyze --arch --scalability

frontend:
  /user:build --react --magic
  /user:test --e2e --pup
  /user:improve --quality --ux

backend:
  /user:build --api --tdd
  /user:analyze --perf --database
  /user:deploy --env prod --plan

security:
  /user:scan --security --strict
  /user:analyze --security --threats
  /user:audit --permissions --access
...
qa:
  /user:test --coverage --interactive
  /user:scan --validate --comprehensive
  /user:analyze --quality --metrics

refactorer:
  /user:improve --quality --solid
  /user:cleanup --code --debt
  /user:analyze --complexity --patterns

analyzer:
  /user:troubleshoot --investigate --five-whys
  /user:analyze --root-cause --systematic
  /user:explain --depth Expert --evidence

mentor:
  /user:explain --depth Beginner --visual
  /user:document --user --examples
  /user:guide --interactive --progressive
```

---

*AI Compass v1.0.0 | 9 specialized cognitive modes | Confidence-integrated expertise | Collaborative intelligence*